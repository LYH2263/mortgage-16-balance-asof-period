...
from app.engines.amortization import balance_asof as balance_asof_engine, equal_payment_schedule
...
class MortgageService:
    ...
    def balance_asof(self, principal, annual_rate, months, target_period, loan_id, persist):
        out = balance_asof_engine(principal, annual_rate, months, target_period)
        rid = None
        if persist:
            rid = runs.insert(self._c, "balance_asof",
                {"principal": principal, "annual_rate": annual_rate, "months": months, "target_period": target_period},
                out, loan_id)
        return {"run_id": rid, **out}
    def dashboard(self):
